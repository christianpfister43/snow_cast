...
def asing_training_cells(_region,data_dir,target,df_train_labels,ground_features,geo, generate_new = False):
    print(f'Asigning training data for region: {_region}')
    #%% creating a list of training / testing cells for the specified region
    cell_ids_train = np.array(df_train_labels['cell_id'])
    cell_ids_test = np.array(target['cell_id'])
    
    cell_ids_train_region = []
    cell_ids_test_region = []
    ...
    for cell_nr in range (len(geo['features'])):
        ...
        if geo['features'][cell_nr]['properties']['region'] == _region:
            if cell_id in cell_ids_train:
                cell_ids_train_region.append(cell_id)
            if cell_id in cell_ids_test:
                cell_ids_test_region.append(cell_id)
    ...
    for cell_nr in range (len(geo['features'])):
        cell_id = geo['features'][cell_nr]['properties']['cell_id']
        if (cell_id in cell_ids_train_region) | (cell_id in cell_ids_test_region):
            for i in range (5):
    ...
    for run in range(1):
        if os.path.isfile(f'{data_dir}/metatdata_newly_generated/cell_asignment_{grid_size}_{_region.replace(" ","_")}_train.pickle'):
            print(f'found existing newly generated file: {data_dir}/metatdata_newly_generated/cell_asignment_{grid_size}_{_region.replace(" ","_")}_train.pickle')
            print('skipping step')
            continue
        elif (not generate_new) & (os.path.isfile(f'{data_dir}/metadata_provided/cell_asignment_{grid_size}_{_region.replace(" ","_")}_train.pickle')):
            print(f'found existing provided file: {data_dir}/metadata_provided/cell_asignment_{grid_size}_{_region.replace(" ","_")}_train.pickle')
            print(f'generating new meta data files is set to  {generate_new} , skipping step')
            continue
        
        for cell_nr in range (len(geo['features'])):
            cell_id = geo['features'][cell_nr]['properties']['cell_id']
            if not cell_id in cell_ids_train_region:
                print(f'Cell with id {cell_id} is not in train region {_region}, moving on')
                continue
            lons = []
            lats = []
            for i in range (5):
        ...
        pickle.dump(grid_to_cell_asignment, open(f'{data_dir}/metatdata_newly_generated/cell_asignment_{grid_size}_{_region.replace(" ","_")}_train.pickle','wb'))
        pickle.dump(grid_mask, open(f'{data_dir}/metatdata_newly_generated/cell_asignment_{grid_size}_{_region.replace(" ","_")}_train_grid_mask.pickle','wb'))
        
        print('#########################################')
        print(f'Finished with asining train cells to grid for region {_region}') 
    ...
    # skips this step if metadata is available or not wanted to generate new
    for run in range(1):
        if os.path.isfile(f'{data_dir}/metatdata_newly_generated/cell_asignment_{grid_size}_{_region.replace(" ","_")}_test.pickle'):
            print(f'found existing newly generated file: {data_dir}/metatdata_newly_generated/cell_asignment_{grid_size}_{_region.replace(" ","_")}_test.pickle')
            print('skipping step')
            continue
        elif (not generate_new) & (os.path.isfile(f'{data_dir}/metadata_provided/cell_asignment_{grid_size}_{_region.replace(" ","_")}_test.pickle')):
            print(f'found existing provided file: {data_dir}/metadata_provided/cell_asignment_{grid_size}_{_region.replace(" ","_")}_test.pickle')
            print(f'generating new meta data files is set to  {generate_new} , skipping step')
            continue
        for cell_nr in range (len(geo['features'])):
            cell_id = geo['features'][cell_nr]['properties']['cell_id']
            if not cell_id in cell_ids_test_region:
                print(f'Cell with id {cell_id} is not in test region {_region}, moving on')
                continue
            lons = []
            lats = []
            for i in range (5):
        ...
        pickle.dump(grid_to_cell_asignment, open(f'{data_dir}/metatdata_newly_generated/cell_asignment_{grid_size}_{_region.replace(" ","_")}_test.pickle','wb'))
        pickle.dump(grid_mask, open(f'{data_dir}/metatdata_newly_generated/cell_asignment_{grid_size}_{_region.replace(" ","_")}_test_grid_mask.pickle','wb'))
    ...
    for run in range(1):
        if os.path.isfile(f'{data_dir}/metatdata_newly_generated/station_asignment_{grid_size}_{_region.replace(" ","_")}.pickle'):
            print(f'found existing newly generated file: {data_dir}/metatdata_newly_generated/station_asignment_{grid_size}_{_region.replace(" ","_")}.pickle')
            print('skipping step')
            continue
        elif (not generate_new) & (os.path.isfile(f'{data_dir}/metadata_provided/station_asignment_{grid_size}_{_region.replace(" ","_")}.pickle')):
            print(f'found existing provided file: {data_dir}/metadata_provided/station_asignment_{grid_size}_{_region.replace(" ","_")}.pickle')
            print(f'generating new meta data files is set to  {generate_new} , skipping step')
            continue
        ...
        pickle.dump(grid_to_cell_asignment_g, open(f'{data_dir}/metatdata_newly_generated/station_asignment_{grid_size}_{_region.replace(" ","_")}.pickle','wb'))
        pickle.dump(grid_mask_g, open(f'{data_dir}/metatdata_newly_generated/station_asignment_{grid_size}_{_region.replace(" ","_")}_grid_mask.pickle','wb'))
